...
Image rgb_to_grayscale(const Image& im)
  ...
  for (int y = 0; y < im.h; y++) {
    for (int x = 0; x < im.w; x++) {
      int rLoc = pixel_address(im, x, y, 0);
      int gLoc = pixel_address(im, x, y, 1);
      int bLoc = pixel_address(im, x, y, 2);
      float r = im.data[rLoc];
      float g = im.data[gLoc];
      float b = im.data[bLoc];

      int grayLoc = pixel_address(gray, x, y, 0);
...
void clamp_image(Image& im)
  ...
  for (int x = 0; x < im.w; x++) {
    for (int y = 0; y < im.h; y++) {
      int loc0 = pixel_address(im, x, y, 0);    
      int loc1 = pixel_address(im, x, y, 1);    
      int loc2 = pixel_address(im, x, y, 2);    
...
void hsv_to_rgb(Image& im)
  ...
  assert(im.c==3 && "only works for 3-channels images");
    for (int x = 0; x < im.w; x++) {
      for (int y = 0; y < im.h; y++) {
        int hLoc = pixel_address(im, x, y, 0);
        int sLoc = pixel_address(im, x, y, 1);
        int vLoc = pixel_address(im, x, y, 2);
